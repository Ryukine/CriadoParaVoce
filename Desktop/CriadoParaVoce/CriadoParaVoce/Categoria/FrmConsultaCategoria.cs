...
{
    public partial class FrmConsultaCategoria : Telas.Modelo.FrmModeloConsulta
    {
        ...
        private void FixarStatus(Object o, EventArgs e)
        {
            try
            {
                ...
                BLL.Categoria obj = new BLL.Categoria(); //<<<<<<<<<<<<<
                obj.CategoriaId = Convert.ToInt32(this.dataGridView1.CurrentRow.Cells[0].Value); //<<<<<<<<
                switch (b.Text)
                {
                    case "Excluir":
                        obj.Excluir();
        ...
        private void AbrirFormulario(object sender, EventArgs e)
        {
            try
            {
                CriadoParaVoce.Categoria.FrmCadastroCategoria f = new CriadoParaVoce.Categoria.FrmCadastroCategoria();
                f.TipoUso = Convert.ToByte(BLL.FuncoesGerais.TipoUso.Inclusao);
                f.txtCodigo.Enabled = false;
                if (sender == this.btnIncluir)
                {
                 //   f.txtUsuario.Visible = false;
                   // f.lblUsuarioSistema.Visible = false;
                    //f.chkGerarLogin.Checked = true;
                    //f.chkGerarLogin.Visible = false;
                    //f.txtSenhaInicial.Visible = false;
                    //f.lblSenhaInicial.Visible = false;
                    f.chkAtivo.Checked = false;
                    //f.chkSenhaAtiva.Visible = false;
                    //f.datCriacao.Enabled = false;
                    //f.lblDataExpiracao.Visible = false;
                    //f.datExpiracao.Value = DateTime.Now.AddMonths(6);
                    //f.datExpiracao.Visible = false;
                    //f.lblDataUltimoAcesso.Visible = false;
                    //f.datUltimoAcesso.Visible = false;
                }
                ...
                if (sender == this.btnAlterar || sender == this.btnConsultar)
                {
                    ...
                    if (sender == this.btnConsultar)
                    {
                        ...
                        f.label1.Text = "Consulta de Categoria";
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            for (; _Linha < this.dataGridView1.Rows.Count; _Linha++)
            {
                ...
                //e.Graphics.DrawString("Seq.: " + _Linha + "  Nome:" + Convert.ToString(this.dataGridView1.Rows[_Linha].Cells[1].Value), _FonteNormal, Brushes.Black, new PointF(_MargemEsquerda, yLinhaTopo));

                //Conteúdo do DataGrid
                e.Graphics.DrawString(Convert.ToString(this.dataGridView1.Rows[_Linha].Cells[0].Value), _FonteNormal, Brushes.Black, new PointF(_MargemEsquerda, yLinhaTopo));
                e.Graphics.DrawString(Convert.ToString(this.dataGridView1.Rows[_Linha].Cells[1].Value), _FonteNormal, Brushes.Black, new PointF(_MargemEsquerda + 40, yLinhaTopo));
